...
var run = function(battery) {
  ...
  var update = function() {
    if (battery.charging) {
      // Chrome keeps saying charging when charging
      // is  actually completed but power is still supplied,
      ...
      if (1 == battery.level && battery.dischargingTime == Infinity) {
        // Hack for Firefox which should let charging being
        // true when charged and power supplied.
        // Best might be another field: 'power-supplied'
        labelMode.textContent = "Charged";
        labelTime.textContent = "";
      } else {
